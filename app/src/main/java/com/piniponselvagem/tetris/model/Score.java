...
public class Score {
	...
	private static final String MUSIC = "tetris_metal_v2";
	...
	public static void drawInfo() {
		if(isSpaceAvailable()) Score.mainUI();	// Draw Score Board
		if (KEYMAP_SHOW && Board.DIM_LINES>=20) Score.keyMap();	// Draw Key Mapping
	}
	*/
	
	public static int isEnabled() {
		if ((SCORE_SHOW || KEYMAP_SHOW) && Board.DIM_LINES>=12) return 22; // 22 is the space required to show the SCORE / KEYMAP.
		return 0;
	}
	
	public static int addLine() {
		// 7 is the default space required to show the default KeyMap
		if (KEYMAP_TXT.length>7 && KEYMAP_SHOW) return (KEYMAP_TXT.length-7);
		return 0;
	}
	...
	public static void nextPiece() {
		if(isSpaceAvailable()) nextPieceShow();
	}
	...
	private static boolean isSpaceAvailable() {
		if (SCORE_SHOW && Board.DIM_LINES>=12) return true;
		return false;
	}
	
	public static void scoreCalc(int point) {
		score+=point;
		if(isSpaceAvailable()) uiPoints(8, 1, score);
	}

	public static void linesCalc(int point) {
		lines+=point;
		if(isSpaceAvailable()) uiPoints(8, 2, lines);
	}
	
	private static void levelCalc(int point) {
		if(isSpaceAvailable()) uiPoints(8, 3, level);
	}
	...
	public static void nextLevel() {
		...
		for (int i=0; i<pieceCount.length ; ++i) {
			count += pieceCount[i]; // counts how many pieces placed to calculate the level
		}
	...
	public static void gamePause() {
		...
		while(Console.isKeyPressed())
			;
		for (int key=NO_KEY ; key!=KeyEvent.VK_P ; TetrisCtrl.nextStepTime+= TetrisCtrl.STEP_TIME/Score.level) {
			key = Console.waitKeyPressed(TetrisCtrl.STEP_TIME/Score.level);
		}
		showPause(false);
	}
	
	private static void showPause(boolean pause) {
		if (pause && isSpaceAvailable()) {
			Console.color(CR_FOREG_PAUSED, CR_BACKG_PAUSED);
			uiText(8,11, "PAUSED");
		}
		else if(!pause && isSpaceAvailable()){
